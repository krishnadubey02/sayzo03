...
const Popular = () => {
  ...
  // id 5 to 10 ke products nikaal lo
  const popularProducts = products.filter(product => product.id >= 5 && product.id <= 10);
    ...
    <div className="bg-white w-full pb-8 px-2 sm:px-5 md:px-7 lg-px-9">
      ...
      <div className="flex flex-wrap justify-between items-center px-9 sm:px-10 md:px-16 py-6">
        <div className="flex gap-4 items-center">
          ...
          <div>
            <h3 className="font-semibold text-2xl text-black">Popular Products</h3>
            <p className="text-xl text-gray-500">Top-rated items for you</p>
          </div>
        </div>
      ...
      <div className="px-7 sm:px-7 md:px-14  grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-5">
        {popularProducts.map((product) => (
          <div
            key={product.id}
            className="flex flex-col gap-2 rounded-lg bg-white px-3 group overflow-hidden hover:scale-105 transition-all duration-500"
          >
            ...
            <div>
              ...
              <div className="flex justify-between items-center mb-3">
                ...
                <button onClick={() => navigate(`/product/${product.id}`)} className="bg-red-600 text-white font-semibold rounded-lg h-9 px-3 text-sm">
                  BUY NOW
                </button>
              </div>
            </div>
